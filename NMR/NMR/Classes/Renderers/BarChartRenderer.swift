...
public class BarChartRenderer: ChartDataRendererBase
{
    ...
    weak var highlightDataProvider: HighlightDataProvider?
    ...
    public func drawDataSet(context context: CGContext, dataSet: IBarChartDataSet, index: Int)
    {
        ...
        for j in 0 ..< Int(ceil(CGFloat(dataSet.entryCount) * animator.phaseX))
        {
            ...
            if (!containsStacks || vals == nil)
            {
                ...
                if let dataProvider = highlightDataProvider {
                    if let index = dataProvider.highlightFromBarIndex(j) {
                        CGContextSetFillColorWithColor(context, dataProvider.highlightColor(index).CGColor)
                    } else {
